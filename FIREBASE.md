...
**الإعداد المطلوب** (`.env.local.example` للتفاصيل): خمس قيم من
Firebase Console → Project settings → General → "Tafalna web"، ونفس
القيم كمتغيّرات مستودع عامة (Actions → Variables) لتبنيها صفحة GitHub
Pages المنشورة. غياب الإعداد لا يكسر التطبيق — بطاقة المزامنة تُعطَّل
بلطف وتبقى بقية الشاشات كما هي.
