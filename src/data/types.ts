...
// ===== دفتر التغييرات (للمزامنة) =====

/**
 * دفتر التغييرات — ذاكرة المزامنة عن «متى» لا «ماذا».
 *
 * بدونه كانت المزامنة تقارن مصفوفتين وتفترض أن الأقصر هو الأحدث، فيكفي
 * أن يفتح جهازٌ التطبيقَ وهو يحمل نسخة أقدم ليمحو ما كتبه الجهاز الآخر.
 * هنا نسجّل لكل عنصر لحظة آخر تعديل عليه، ولكل عنصر محذوف لحظة حذفه —
 * فيصير الفرق بين «لم يصلني هذا العنصر بعد» و«حُذف عمدًا» معلومة صريحة
 * لا تخمينًا.
 *
 * المفاتيح كلها معرّفات عناصر (`item.id`)، والقيم لحظات ISO.
 */
export interface SyncMeta {
  /** معرّف العنصر ← لحظة آخر تعديل عليه */
  rev: Record<string, string>
  /** معرّف العنصر ← لحظة حذفه (شاهدة قبر تُبلِّغ الجهاز الآخر بالحذف) */
  deleted: Record<string, string>
  /** لحظة آخر تعديل على ملف الطفل — ليس عنصرًا في مجموعة فله حقله */
  childRev?: string
}
...
export interface AppData {
  ...
  /** دفتر التغييرات — يُملأ تلقائيًا مع كل تعديل، ولا تقرؤه أي شاشة */
  syncMeta: SyncMeta
}
